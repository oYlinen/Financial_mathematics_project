function[J] = jacobian(h,par0,robust)

% robust "off" (default if robust option is omitted): Uses martin's function for comupting Jacobian, works but 
% is a quick implementation and might be inaccurate
% robust "on": uses an ad-hoc function found online on mathworks

% h:        Is a function handle of parameters only
% par0:     Estimates of the parameters
...
switch nargin
    ...
    case 3
        ...
        switch robust
            case 'on'
                J = jacobianest(h,par0);
            case 'off'
                J = numJ(h,par0,1e-6);
            otherwise
                disp('Robust option is either "on" or "off".')
        end
end
end

function[jac] = numJ(h,par0,epsilon)
% h:        Is a function handle of parameters only
% par0:     Estimates of the parameters
% epsilon:  Step for evaluating the derivatives:
nx = numel(par0);
f0 = h(par0);
jac = NaN(numel(f0),nx);
